...
# The engine logs nothing on a successful shader compile, so "no 'Failed to compile
# shader' line" is the pass signal.
set -u
...
if grep -h -A12 "Failed to compile shader\|Could not find shader file" "$LOGS/error.log" "$LOGS/game.log" 2>/dev/null | head -80 | grep .; then
	exit 1
else
	echo "none"
fi
